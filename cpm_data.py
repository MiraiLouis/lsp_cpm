...
class LSPDataset(Dataset):
    def __init__(self, root_dir, transform=None, phase_train=True,
        ...
        if lms is None or imagefiles is None or weight is None:
            ...
            mat_lspet = scipy.io.loadmat(os.path.join(root_dir, 'joints.mat'),
            ...
            # Trier les images par ordre numérique basé sur leur nom
            image_nums_lspet = np.array([float(os.path.basename(s)[2:-4]) for s in image_lspet])
            sorted_image_lspet = image_lspet[np.argsort(image_nums_lspet)]

            # Extraire uniquement les coordonnées x, y (pas le canal de visibilité)
            self.lms = mat_lspet.transpose([2, 1, 0])[:, :2, :]

            self.imagefiles = sorted_image_lspet

            # Calcul des tailles d'images pour redimensionnement des annotations
            imgs_shape = []
            ...
            # Créer un masque de poids basé sur les points clés valides
            ...
            # Appliquer la pondération basée sur KDE si activé
            ...
            # Stocker les données globalement pour éviter de les recharger
            lms = self.lms
            imagefiles = self.imagefiles
            weight = self.weight
        else:
            # Recharger les données globales si elles existent déjà
            self.lms = lms
